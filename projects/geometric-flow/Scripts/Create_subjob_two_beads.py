...
def Create_json_wrapping_two(ka,kb,r,inter_str,angle):
    ...
    d1 = Rpos_beads*np.cos(theta) - np.sqrt(1.6*1.6 - Rpos_beads*Rpos_beads*np.sin(theta)**2)
    d2 = Rpos_beads*np.cos(theta) - np.sqrt(1.5*1.5 - Rpos_beads*Rpos_beads*np.sin(theta)**2)  
    disp = (d1+d2)/2.0
