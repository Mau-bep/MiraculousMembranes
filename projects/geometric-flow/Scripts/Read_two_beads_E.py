...
def main(theta):
    ...
    plt.plot(Data[:,3]/(20.0*4*np.pi),color='black',label='Total E')
    plt.plot(Data2[:,3]/(20.0*4*np.pi),color='blue',label='Total E with switch')
    plt.plot(Data3[:,3]/(20.0*4*np.pi),color='red',label='Total E with switch no remesh')
    plt.plot(Data4[:,3]/(20.0*4*np.pi),color='green',label='Total E longersim')
    plt.legend()
