...
int main(int argc, char **argv)
{
    ...
    SparseMatrix<double> Hessian_Ben_mesh = Sim_handler.H_Bending_tan(Energy_constants[0]);
    std::cout << "The sparse hessian for bending is \n"
              << Hessian_Ben_mesh << "\n";

    Eigen::MatrixXd Hessian_Ben_finite_mesh(3 * mesh->nVertices(), 3 * mesh->nVertices());
    ...
    for (Vertex v : mesh->vertices())
    {
        ...
        for (size_t coord = 0; coord < 3; coord++)
        {
            ...
            geometry->inputVertexPositions[v][coord] += 1e-6; // move forward
            geometry->refreshQuantities();
            Gradient_fwd = Sim_handler.F_Bending_tan(Energy_constants[0]);
            geometry->inputVertexPositions[v][coord] -= 2e-6; // move backward
            geometry->refreshQuantities();
            Gradient_prev = Sim_handler.F_Bending_tan(Energy_constants[0]);
            geometry->inputVertexPositions[v][coord] += 1e-6; // restore
            geometry->refreshQuantities();
            ...
            Hessian_Ben_finite_mesh.col(dim) = Difference_grad;
            dim += 1;
        }
    }

    std::cout << "The finite difference hessian is \n"
              << Hessian_Ben_finite_mesh << "\n";
    ...
    Eigen::MatrixXd DifferenceHessians = Hessian_Ben_finite_mesh + Hessian_Ben_mesh;
    std::cout << "The matrix difference between the hessians is somethign like \n"
              << DifferenceHessians << "\n";
    ...
    // Eigen::MatrixXd DifferenceHessians = Hessian_finite_diff + Hessian_bending;
    DifferenceHessians = Hessian_Edge_reg_finite_mesh + Hessian_Edge_reg_mesh;
    ...
    Eigen::MatrixXd Hessian_Bead = Bead_1.Bead_I->Hessian_IP();
    ...
    std::cout << "THe Hessian of the Bead interaction with finite differences is \n"
              << Hessian_Bead_finite << "\n";
    std::cout << " The Hessian of the bead is \n"
              << Hessian_Bead << "\n";
