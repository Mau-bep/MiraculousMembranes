...
//
double CalculateCoverage(std::string Simdir, int Step, Vector3 BeadPos, double radius, double rc)
{
    std::tie(mesh_uptr, geometry_uptr) = readManifoldSurfaceMesh(Simdir + "membrane_" + std::to_string(Step) + ".obj");
    mesh = mesh_uptr.release();
    geometry = geometry_uptr.release();

    // Now we need to calculate the coverage
    double A_covered = 0.0;
    double FaceArea;
    Vector3 Normal;
    Vector3 rij;
    Vector3 Vert_pos;
    ...
    for (Face f : mesh->faces())
    {
        ...
        for (Vertex vi : f.adjacentVertices())
        {
            ...
            rij = (BeadPos - Vert_pos);
            r_dist = rij.norm();
            rij = rij.unit();

            if ((dot(rij, Normal) > 0.0 && r_dist < std::min(rc, radius * 1.3)))
            {
                A_covered += FaceArea / 3.0;
...
std::vector<double> ReadCoverage(std::string Subdir)
{
    ...
    JsonFile.open(Subdir + "Input_file.json", std::ios::in);
    //
    if (!JsonFile.is_open())
    {
        std::cout << "ERROR OPENING THE FILE D:\n";
        return Coverage_data;
    }
    json Data = json::parse(JsonFile);
    // Ok so we parsed the data, which numbers do you need
    // Ineed KA, KB, Bead radius and interaction strenght
    double KA = 0;
    double KB = 0;
    for (auto Energy : Data["Energies"])
    {
        ...
        if (Energy["Name"] == "Surface_tension" || Energy["Name"] == "Membrane_tension" || Energy["Name"] == "Excess_tension")
        {
            KA = Energy["constants"][0];
        }
    }
    // I now need the bead radius
    double r = 0;
    double rc = 0;
    double KI = 0;
    ...
    // I need the bead position so i need to read the bead file
    std::ifstream Bead_data(Subdir + "Bead_0_data.txt");
    std::string line;
    int StepCurrent = 0;
    Vector3 BeadPos;
...
int main(int argc, char **argv)
{
    ...
    std::string SimsDir = Data["first_dir"];

    DIR *FD;
    struct dirent *in_file;
    const char *target_dir = SimsDir.c_str();
    ...
    while ((in_file = readdir(FD)))
    {
        ...
        if (strlen(in_file->d_name) < 3 && isdigit(in_file->d_name[0]))
        {
            ...
            std::string basic_name = SimsDir + in_file->d_name + "/";
            // Ok now i am at the directory where i want to be
            Subdirs.push_back(basic_name);
        }
    }
