  ...
  // std::cout<<"IS is related to beads?\n";
    ...
    for(size_t bi = 0; bi < Beads.size(); bi++){
      if(Beads[bi]->state == "manual"){
        ...
        if( Bpos.norm2()< (2.0-1.0*Beads[bi]->sigma)*(2.0-1.0*Beads[bi]->sigma) || Bpos.norm2()> (2.0+1.0*Beads[bi]->sigma)*(2.0+1.0*Beads[bi]->sigma )) //The 2.0 here is hardcoded and it means the radius of the vesicle
