...
VertexData<Vector3> E_Handler::F_Bending(std::vector<double> Constants) const
{
    ...
    for (Face f : mesh->faces())
    {
        ...
        constant = -1 * (Scalar_MC[Vertices_face[0]] * Scalar_MC[Vertices_face[0]] / (Dual_areas[Vertices_face[0]] * Dual_areas[Vertices_face[0]]) - H0 * H0) / 3.0 - 1 * (Scalar_MC[Vertices_face[1]] * Scalar_MC[Vertices_face[1]] / (Dual_areas[Vertices_face[1]] * Dual_areas[Vertices_face[1]]) - H0 * H0) / 3.0 - 1 * (Scalar_MC[Vertices_face[2]] * Scalar_MC[Vertices_face[2]] / (Dual_areas[Vertices_face[2]] * Dual_areas[Vertices_face[2]]) - H0 * H0) / 3.0;
...
VertexData<Vector3> E_Handler::F_Bending_tan(std::vector<double> Constants) const
{
    ...
    {
        Dual_areas[v] = geometry->barycentricDualArea(v);   // Ai
        Scalar_MC[v] = geometry->scalarMeanCurvatureTan(v); // Hi
    }
    ...
    for (Edge e : mesh->edges())
    {
        ...
        dih = geometry->dihedralAngle(e.halfedge());
        lij = geometry->edgeLength(e);
        tan_dih = 2 * tan(0.5 * dih);

        Grad_dihedral *= (1 + 0.25 * tan_dih * tan_dih);
    ...
    for (Face f : mesh->faces())
    {
        ...
        for (size_t i = 0; i < 3; i++)
        {
            ...
            if (Vertices[i].isBoundary())
            {
                ...
                Force[Vertices[i]] += -1 * KF * Force_vector * (A - Face_reference[f]) / (Face_reference[f] * Face_reference[f]);
        }
    }
    ...
    for (Face f : mesh->faces())
    {
        ...
        for (int i = 0; i < 9; i++)
        {
            for (int j = 0; j < 9; j++)
            {
                ...
                if (Hessian_block(i, j) > 1e-12 || Hessian_block(i, j) < -1e-12)
                    tripletList.push_back(T(indices[i / 3] * 3 + i % 3, indices[j / 3] * 3 + j % 3, Hessian_block(i, j)));
                if (Hessian_block_2(i, j) > 1e-12 || Hessian_block_2(i, j) < -1e-12)
                    tripletList.push_back(T(indices[i / 3] * 3 + i % 3, indices[j / 3] * 3 + j % 3, Hessian_block_2(i, j)));
            }
        }
    }
    Hessian.setFromTriplets(tripletList.begin(), tripletList.end());
...
SparseMatrix<double> E_Handler::Calculate_Hessian_E_Normal()
{
    ...
    for (int k = 0; k < Full_hessian.outerSize(); ++k)
        for (SparseMatrix<double>::InnerIterator it(Full_hessian, k); it; ++it)
        {
            ...
            if (visited_pairs.find(current_pair) != visited_pairs.end())
            {
                // std::cout << "The pair with row " << current_pair.first << " and column " << current_pair.second << " has been done already\n";
                // This means that the pair has already been visited and we can skip it
                continue;
            }
            else if (current_pair.first >= N_verts || current_pair.second >= N_verts)
            {
                ...
                // Block = Full_hessian.block(row
                // Now the thing is that i only need the one that is bigger
                if (current_pair.first >= N_verts && current_pair.second < N_verts)
                {
                    ...
                    Bead_id = (row / 3) - N_verts;
                    if (Bead_id >= 1)
                        std::cout << "The bead id is " << Bead_id << " \n";

                    for (int i = 0; i < 3; i++)
                    {
                        tripletList.push_back(T(N_verts + Bead_id * 3 + i, current_pair.second, Row_vec[i]));
                        tripletList.push_back(T(current_pair.second, N_verts + Bead_id * 3 + i, Row_vec[i]));
                    }
                    continue;
                }
                // if (current_pair.first < N_verts && current_pair.second >= N_verts)
                ...
                if (current_pair.first >= N_verts && current_pair.second >= N_verts)
                {
                    // This means that we are in the bead-bead part of the hessian and we can skip it
                    // Here i need to add this elements better lol
                    Block = Full_hessian.block(3 * current_pair.first, 3 * current_pair.second, 3, 3);
                    // I need to copy this entire block
                    for (int i = 0; i < 3; i++)
                    {
                        for (int j = 0; j < 3; j++)
                        {
                            tripletList.push_back(T(3 * (row / 3 - N_verts) + N_verts + i, 3 * (col / 3 - N_verts) + N_verts + j, Block(i, j)));
                        }
                    }
                    // std::cout << "This one is true\n";
                }
            }
            else
            {
                ...
                Normal_i << Vertex_normals[row / 3].x, Vertex_normals[row / 3].y, Vertex_normals[row / 3].z;
                Normal_j << Vertex_normals[col / 3].x, Vertex_normals[col / 3].y, Vertex_normals[col / 3].z;
                Block = Full_hessian.block(3 * (row / 3), 3 * (col / 3), 3, 3);
                val = Normal_i.transpose() * Block * Normal_j;
                tripletList.push_back(T(row / 3, col / 3, val));
            }
            // And this is how u turn the block into the thing
...
SparseMatrix<double> E_Handler::Calculate_Hessian_Constraints()
{
    ...
    // For beads you need to include the beads at the size of the matrix of the hessian
    // I recommend adding them at the bottom.
    // std::cout<<"We are here and we are queer\n";
    int N_verts = mesh->nVertices();
    int N_beads = Beads.size();
    // std::cout<<"THe number of beads is " << N_beads << "\n";
    ...
    SparseMatrix<double> Hessian(3 * (N_verts + N_beads), 3 * (N_verts + N_beads));
    std::string constraint;
    if (Lagrange_mult.size() == 0)
    {
        Lagrange_mult = Eigen::VectorXd::Zero(Constraints.size());
    }
