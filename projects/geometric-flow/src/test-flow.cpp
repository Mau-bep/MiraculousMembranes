...
TEST_F(MeanCurvatureFlowTest, laplaceMatrixSymmetric) {
    ...
    SparseMatrix<double> A2 = MCF.geometry->laplaceMatrix2();

    std::cerr << "\n\nTHe difference between the two matrices is "<<(A-A2).norm() << " Which maybe is not that bad? \n" << std::endl;
            std::runtime_error("");
...
TEST_F(MeanCurvatureFlowTest, integrate) {
    ...
    for (Vertex v : MCF.mesh->vertices()) {
        //  std::cerr << "\n\nMy result is "<<(MCF.geometry->inputVertexPositions[v] - mcf_soln[v.getIndex()]).norm() << " Which maybe is not that bad? \n" << std::endl;
            // std::runtime_error("");
        
        if(v.getIndex()==0){
    
            std::cerr << "\n\nThe expected result is "<<mcf_soln[v.getIndex()] <<" And i got" <<  MCF.geometry->inputVertexPositions[v] << std::endl;
            std::runtime_error("");
        }
