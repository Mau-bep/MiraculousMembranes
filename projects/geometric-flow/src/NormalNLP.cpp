...
//    std::cout<<"The number of constraints is "<< M3DG->Sim_handler->Constraints.size() << "\n";
   ...
   nnz_jac_g = m*n; //I need to calculate this
...
)
{
   ...
   Index idx = 0;
    ...
    for(int i = 0; i < n; i++){
        x[i] = 0.0; // Start with no normal displacement
    }
...
)
{
    ...
    for(int i = 0; i < n; i++){
        grad_f[i] = dot(Current_grad[i],Normals[i]); // Start with zero gradient
    }
...
)
{
    ...
    else
    {
        ...
        for(size_t i = 0; i < n; i++){
            values[idx++] = dot(volGrad[i],Normals[i]);
        ...
        for(size_t i = 0; i < n; i++){
            values[idx++] = dot(areaGrad[i],Normals[i]);
        }
...
)
{
    ...
    else
    {
        ...
        Eigen::SparseMatrix<double> Hessian_temp; 
        if(m == 1) Hessian_temp = obj_factor * M3DG->Sim_handler->Calculate_Hessian_E_Normal() + lambda[0]*M3DG->Sim_handler->H_Volume_Normal(std::vector<double>{1.0}); //+ obj_factor* M3DG->Sim_handler->Beads[0]->Bead_I->Hessian();
        if(m == 2) Hessian_temp = obj_factor * M3DG->Sim_handler->Calculate_Hessian_E_Normal() + lambda[0]*M3DG->Sim_handler->H_Volume_Normal(std::vector<double>{1.0}) + lambda[1]*M3DG->Sim_handler->H_SurfaceTension_Normal({1.0});
