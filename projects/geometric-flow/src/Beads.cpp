    ...
    if(interaction=="Shifted_LJ_Normal_nopush" || interaction == "Frenkel_Normal_nopush"){
        ...
        for(Face f : mesh->faces()){
            ...
            for(Halfedge he : f.adjacentHalfedges()){
                ...
                if(dot(unit_r,Face_normal) < 0){
                    Force[v1_idx]+=-1*(1.0/3.0)*E_v[v1_idx]*dot(Face_normal,unit_r)*Grad_vec;
                }
                if(dot(unit_r2,Face_normal) < 0){
                    Force[v1_idx]+=-1*(1.0/3.0)*E_v[v2_idx]*dot(Face_normal,unit_r2)*Grad_vec;
                }
                if(dot(unit_r3,Face_normal) < 0){
                    Force[v1_idx]+=-1*(1.0/3.0)*E_v[v3_idx]*dot(Face_normal,unit_r3)*Grad_vec;
                }
                ...
                F=-1*dot(Face_normal,unit_r)*(-2)*(Face_area/3)*strength*alpha*(  (sigma*sigma/(r*r*r))*pow(( (rc2/(r*r))-1),2 )+ 2*((sigma*sigma/(r*r))-1)*( (rc2/(r*r))-1 )*(rc2/(r*r*r)) )*unit_r;
                // F=(Face_area/3)*dot(Face_normal,unit_r)*strength*alpha*(-2)*(  (sigma*sigma/(r*r*r))*pow(( (rc2/(r*r))-1),2 )+ 2*((sigma*sigma/(r*r))-1)*( (rc2/(r*r))-1 )*(rc2/(r*r*r)) )*unit_r;
                Total_force-=F;
                Force[v1_idx]+=F;
                }
                ...
                // if(true){
                    ...
                    Force[v1_idx]+=-1*(Face_area/3.0)*E_v[v1_idx]*grad;    
                    Force[v1_idx]+=-1*(Face_area/3.0)*E_v[v1_idx]*(Face_normal-unit_r*dot(unit_r,Face_normal))/(r);
                
                    Total_force-=-1*(Face_area/3.0)*E_v[v1_idx]*(Face_normal-unit_r*dot(unit_r,Face_normal))/(r);
                }
                // Vertex 2
                ...
                // if(true){
                    ...
                    Force[v1_idx]+=-1*(Face_area/3.0)*E_v[v2_idx]*grad;
                }
                // Force[v1_idx]+=grad;
                ...
                // if(true){
                    ...
                    Force[v1_idx]+=-1*(Face_area/3.0)*E_v[v3_idx]*grad;
                }
                // Force[v1_idx]+=grad;
