...
VertexData<Vector3> Mem3DG::buildFlowOperator(double h, double V_bar, double nu, double c0,double P0,double KA,double KB, double Kd)  {
    ...
    for(Vertex v : mesh->vertices()){
      ...
      H_target[index]= (system_time<50? [index]+dH_Vector[index]*system_time: H_bar);
    }
...
VertexData<Vector3> Mem3DG::Bending(double H0,double KB, double Kd) const {
    ...
    // #pragma omp parallel for private(F1,F2,F3,F4,Position_1,neigh_index,Position_2,e_ji,Kij,factor,Hij,Sij_1,Sij_2)
...
double Mem3DG::E_Bending(double H0,double KB) const{
    ...
    for(Vertex v : mesh->vertices()) {
        ...
        H=abs(geometry->scalarMeanCurvature(v)/geometry->barycentricDualArea(v)-(system_time<50? H_Vector_0[index]+dH_Vector[index]*system_time: H0));
...
*/
double Mem3DG::Backtracking(VertexData<Vector3> Force,double D_P,double V_bar,double A_bar,double KA,double KB,double H_bar) const {
double c1=5e-4;
double rho=0.7;
double alpha=1e-3;
