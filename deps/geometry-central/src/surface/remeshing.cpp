  ...
  while (!toSplit.empty()) {
    ...
    if (length_e > minLength && (length_e > threshold *2.0 ) ) {
      ...
      if (he != Halfedge()) {
        ...
        // std::cout<< "The vertex we added is   "<<he.vertex().getIndex()<< "With position"
        // <<geom.inputVertexPositions[he.vertex()] <<"\n\n\n" ;

        if (!std::isnan(newPos.x)) {
          // MAU ADDED THIS (not enough for debugging the whole )
